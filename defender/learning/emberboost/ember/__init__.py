...
def vectorize_subset(X_path, y_path, raw_feature_paths, extractor, nrows):
    ...
    doinparallel = True # if False, we can debug more easily.
    if doinparallel is True:
        ...
        argument_iterator = ((irow, raw_features_string, X_path, y_path, extractor, nrows)
                             for irow, raw_features_string in enumerate(raw_feature_iterator(raw_feature_paths)))
        for arg in argument_iterator:
            vectorize_unpack(arg)


def create_vectorized_features(data_dir, feature_version=2,
                               extractor: typing.Optional[PEFeatureExtractor] = None):
    """
...
def subselect(X_dat, y_dat, nosamples):
    """
    My custom function, to reduce number of training samples
    """
    xpos = sample_without_replacement(X_dat.shape[0], nosamples, random_state=31)
    xpos = np.sort(xpos)

    return X_dat[xpos], y_dat[xpos]
...
def optimize_model(data_dir):
    ...
    X_train, y_train = subselect(X_dat = X_train, y_dat= y_train, nosamples=5000)
